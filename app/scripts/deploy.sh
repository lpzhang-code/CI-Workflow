...
if [[ -z "$PRIVATE_SSH_KEY" ]]
then
    echo -e "\n>>> Error: private key not found in CLERK_PRIVATE_SSH_KEY."
    exit 1
fi

echo -e "\n>>> Deploying $PROJECT to $HOST with suffix $COMPOSE_SUFFIX."

# Set up SSH key
echo -e "\n>>> Setting up private key."
echo -e "$PRIVATE_SSH_KEY" > deploy.key
...
echo -e "\n>>> Copying $PROJECT compose file to $HOST"
